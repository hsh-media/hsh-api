...
with open('florence_sept_opens.csv', 'w', newline = '') as csv_out:
    ...
    for f in files:
        ...
        for obj in bucket.objects.filter(Prefix=f):
            ...
            for line in body.splitlines():
                ...
                if json_data['eventType'] == 'Open':
                    opens.append(json_data['mail']['destination'][0])

    openers = list(set(opens))

    for email in openers:
        csv_writer.writerow([email, 'Opened'])